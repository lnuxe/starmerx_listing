"""临时探针：精读 col_69(活动营销费) 与 col_58(利润/利润率) 列内的所有 input。"""
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        # 精读 col_69 和 col_58 的所有 input 及其后缀/标签
        dump = page.evaluate(
            """() => {
                const roots = [...document.querySelectorAll(
                    '.el-drawer:not([style*="display: none"]), '
                ...
                const out = {};
                for (const cid of ['col_58', 'col_69', 'col_68', 'col_70']) {
                    const td = root.querySelector(`td[colid="${cid}"]`);
                    if (!td) { out[cid] = 'no td'; continue; }
                    out[cid] = {
                        text: td.innerText,
                        html_snippet: td.innerHTML.slice(0, 1500),
                        inputs: [...td.querySelectorAll('input')].map(e => ({
                            value: e.value,
                            type: e.type,
                            ph: e.placeholder || '',
                            suffix: (e.closest('.el-input') || e.parentElement)?.innerText || '',
                        })),
                    };
                }
                return out;
        ...
        shot = ROOT / "runtime" / "probe_col69_detail.png"
