...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        # 精确映射：遍历滚动表首行每个 td，读 td 内 input 及该 td 对应的列头
        dump = page.evaluate(
            """() => {
                const roots = [...document.querySelectorAll(
                    '.el-drawer:not([style*="display: none"]), '
                ...
                for (const t of root.querySelectorAll('.vxe-table')) {
                    const heads = [...t.querySelectorAll('.vxe-header-column .vxe-cell, th')]
                        .map(h => (h.innerText||'').trim()).filter(Boolean);
                    if (heads.includes('活动营销费($) / 占比')) { tb = t; break; }
                }
                ...
                // 首行数据 td 顺序
                const row = [...tb.querySelectorAll('.vxe-body--row, tr.vxe-row, tbody tr')]
                    .find(r => r.offsetParent !== null);
                if (!row) return { note: 'no row' };

                const cells = [...row.querySelectorAll('td')];
                const mapped = cells.map((td, i) => {
                    const inp = td.querySelector('input');
                    return {
                        col: i,
                        header: headers[i] || '(none)',
                        input_value: inp ? inp.value : null,
                        input_ph: inp ? (inp.placeholder||'') : null,
                    };
                });
        ...
        shot = ROOT / "runtime" / "probe_drawer_map.png"
